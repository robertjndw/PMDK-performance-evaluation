...
RESULT_PATH=$RESULT_PATH/postgres-original
mkdir -p $RESULT_PATH
...
sudo sed -i 's/#wal_sync_method = pmem_drain/wal_sync_method = fsync/g' $MOUNT_PM/pgsql_data/postgresql.conf
sudo -u postgres /usr/local/pgsql/bin/pg_ctl -s -D $MOUNT_PM/pgsql_data start
