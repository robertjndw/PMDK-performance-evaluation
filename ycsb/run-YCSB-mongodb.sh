...
RESULT_PATH=$RESULT_PATH/mongodb
mkdir -p $RESULT_PATH
...
(cd $MONGODB_PATH && PMEM_IS_PMEM_FORCE=1 ./mongod --storageEngine=pmse --dbpath=$DATABASE_PATH --fork --syslog &) > /dev/null
sleep 1
...
(cd $MONGODB_PATH && pkill mongod) > /dev/null
sudo rm -rf $DATABASE_PATH
sleep 1
echo "##### YCSB mongodb DONE #####"
